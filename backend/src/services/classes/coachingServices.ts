...
export const getVacanciesService = async (
  professorId: number,
  schoolYearId: number,
) => {
...
export const requestCoachingService = async (body: unknown) => {
  ...
  if (assistant_professor_id) {
    const assistant = await prisma.user.findUnique({
      where: { userId: assistant_professor_id },
    });
    if (!assistant)
  ...
  if (!studioModality)
    throw new AppError("Estúdio não disponível para esta modalidade.", 404);
  ...
  if (assistant_professor_id) {
    await addUserToClassService(
      ...
      {
        userId: assistant_professor_id,
        userClassRoleId: assistantRole.userClassRoleId,
      },
    );
  }
